# Cactus Engine FFI API Reference - Complete Documentation

## Overview

The Cactus Engine provides a C FFI for integrating LLM inference into applications. This documentation covers all available functions, parameters, and usage examples.

## Getting Started

Download model weights before use:
...
Weights are saved to `weights/` directory. Moonshine requires FP16 precision.

## Core Types

### `cactus_model_t`
An opaque pointer representing a loaded model instance.
...
### `cactus_index_t`
An opaque pointer representing a vector index instance.
...
An opaque pointer representing a streaming transcription session.
...
Callback function for streaming token generation:

```c
typedef void (*cactus_token_callback)(
    const char* token,      // Generated token text
    uint32_t token_id,      // Token's ID in vocabulary
...
```c
typedef void (*cactus_log_callback_t)(
    int level, 
...
cactus_model_t cactus_init(
    ...
    const char* corpus_dir,   // Optional corpus directory for RAG (NULL ok)
    bool cache_index          // false = rebuild, true = load cached
);
```
...
int cactus_complete(
    ...
    const char* options_json,       // Optional generation options (NULL ok)
    const char* tools_json,         // Optional tools definition (NULL ok)
    cactus_token_callback callback, // Optional streaming callback (NULL ok)
    void* user_data,                // User data for callback (NULL ok)
    const uint8_t* pcm_buffer,      // Optional raw PCM audio buffer (NULL ok)
...
**Messages with Images (VLM models):**
...
**Messages with Audio (multimodal models like Gemma4):**
...
```json
[
    {"role": "user", "content": "Describe and transcribe.", "images": ["/path/to/image.jpg"], "audio": ["/path/to/audio.wav"]}
]
```
...
| `min_p` | float | 0.15 | Minimum probability threshold |
| `repetition_penalty` | float | 1.1 | Penalize previously generated tokens |
| `stop_sequences` | array | [] | Stop generation on these strings |
| `include_stop_sequences` | bool | false | Include stop sequence tokens |
| `force_tools` | bool | false | Constrain output to tool call format |
| `tool_rag_top_k` | int | 2 | Select top-k relevant tools |
| `confidence_threshold` | float | 0.7 | Minimum confidence for local generation |
| `auto_handoff` | bool | true | Auto cloud handoff on low confidence |
| `cloud_timeout_ms` | int | 15000 | Cloud handoff timeout (milliseconds) |
| `handoff_with_images` | bool | true | Allow cloud handoff with images |
| `enable_thinking_if_supported` | bool | false | Enable chain-of-thought thinking blocks |
...
When `cloud_handoff` is true, confidence dropped below the threshold and the response was fulfilled by a cloud model.
...
Note: `ram_usage_mb` reflects actual RAM usage even in error responses.
...
Pre-processes input text and populates the KV cache without generating output tokens.
...
**Message and Options Format:** Same as `cactus_complete` (see above)
...
int cactus_tokenize(
    ...
    size_t token_buffer_len,     // Maximum number of tokens
    size_t* out_token_len        // Output: actual number of tokens
);
```

**Returns:** 0 on success; -1 on invalid parameters or tokenization error; -2 if buffer is too small (but `*out_token_len` is set to required count). Pass NULL for `token_buffer` and 0 for `token_buffer_len` to query token count without copying.
...
int cactus_transcribe(
    cactus_model_t model,           // Model handle
    const char* audio_file_path,    // Path to WAV file (16-bit PCM) - NULL ok
    const char* prompt,             // Optional transcription prompt (NULL ok)
    char* response_buffer,          // Buffer for response JSON
    size_t buffer_size,             // Size of response buffer
    const char* options_json,       // Optional transcription options (NULL ok)
    cactus_token_callback callback, // Optional streaming callback (NULL ok)
    void* user_data,                // User data for callback (NULL ok)
    const uint8_t* pcm_buffer,      // Optional raw PCM audio buffer (NULL ok)
    size_t pcm_buffer_size          // Size of PCM buffer in bytes
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `max_tokens` | int | auto | Maximum tokens to generate |
| `temperature` | float | 0.0 | Sampling temperature |
| `top_p` | float | 0.0 | Top-p (nucleus) sampling |
| `top_k` | int | 0 | Top-k sampling |
| `use_vad` | bool | true | Split audio using voice activity detection |
| `cloud_handoff_threshold` | float | model default | Maximum token entropy norm for cloud handoff |
| `custom_vocabulary` | array | [] | Words or phrases to boost recognition |
| `vocabulary_boost` | float | 5.0 | Log-probability bias for custom vocabulary tokens |
...
- `segments`: Array of `{"start": float, "end": float, "text": string}` with timestamps (seconds). Whisper produces phrase-level segments from timestamp tokens; Parakeet TDT produces word-level segments; Parakeet CTC and Moonshine produce one segment per transcription window.
- `cloud_handoff`: true when threshold exceeded and text is non-empty and longer than 5 characters
...
| `custom_vocabulary` | array | [] | Words or phrases to bias decoder toward |
| `vocabulary_boost` | float | 5.0 | Logit bias strength (0.0–20.0) |
...
cactus_stream_transcribe_t cactus_stream_transcribe_start(
    cactus_model_t model,        // Model handle
    const char* options_json     // Optional configuration (NULL ok)
);
```
...
- `min_chunk_size`: Minimum audio samples before transcription processing (default: 32000)
- `language`: ISO 639-1 language code (e.g., "en", "es", "fr", "de"). Default: "en". Ignored for non-Whisper models.
- `custom_vocabulary`: Words or phrases to bias the decoder. Applied for lifetime of stream.
- `vocabulary_boost`: Logit bias strength (default: 5.0, clamped to 0.0–20.0)
...
When accumulated audio has not reached `min_chunk_size`, a minimal buffering response is returned immediately (no inference run):
...
- `buffer_duration_ms`: Duration of confirmed audio consumed from buffer (milliseconds)
- `confirmed`: Confirmed transcription from this chunk; may reflect cloud transcript if used
- `confirmed_local`: Confirmed text from local model (before cloud override)
- `pending`: Current (not yet confirmed) transcription result
- `segments`: Array of `{"start": float, "end": float, "text": string}` with timestamps (seconds, relative to stream start)
- `cloud_handoff`: Whether a cloud transcription job was queued
- `cloud_job_id`: ID of queued cloud job (0 if none)
- `cloud_result_job_id`: ID of cloud job whose result is returned (0 if none ready)
- `cloud_result`: Transcript from completed cloud job
- `cloud_result_used_cloud`: Whether the cloud job reached a cloud API
- `cloud_result_error`: Error from cloud job
- `cloud_result_source`: `"cloud"` or `"fallback"` for completed cloud job
...
int cactus_stream_transcribe_stop(
    cactus_stream_transcribe_t stream,  // Stream handle
    char* response_buffer,              // Buffer for response JSON (NULL ok)
    size_t buffer_size                  // Size of response buffer (0 ok)
);
```
...
int cactus_diarize(
    ...
    char* response_buffer,          // Buffer for response JSON
    size_t buffer_size,             // Size of response buffer
    const char* options_json,       // Optional JSON options (NULL ok)
    const uint8_t* pcm_buffer,      // Optional raw int16 PCM buffer (NULL ok)
    size_t pcm_buffer_size          // Size of PCM buffer in bytes
);
```
...
|-------|------|---------|-------------|
| `step_ms` | int | 1000 | Sliding window stride in milliseconds |
| `threshold` | float | none | Zeroes out per-speaker scores below this value |
| `num_speakers` | int | none | Keep N most active speakers |
| `min_speakers` | int | none | Lower bound on active speakers |
| `max_speakers` | int | none | Upper bound on active speakers |
| `raw_powerset` | bool | false | Return raw 7-class powerset scores |
...
int cactus_embed_speaker(
    ...
    char* response_buffer,          // Buffer for response JSON
    size_t buffer_size,             // Size of response buffer
    const char* options_json,       // Optional JSON options (NULL ok, reserved)
    const uint8_t* pcm_buffer,      // Optional raw int16 PCM buffer (NULL ok)
    size_t pcm_buffer_size,         // Size of PCM buffer in bytes
    const float* mask_weights,      // Optional per-frame mask weights (NULL ok)
    size_t mask_num_frames          // Number of mask weight frames (0 if NULL)
);
```
...
int cactus_detect_language(
    ...
    char* response_buffer,          // Buffer for response JSON
    size_t buffer_size,             // Size of response buffer
    const char* options_json,       // Optional options (NULL ok)
    const uint8_t* pcm_buffer,      // Optional raw PCM audio buffer (NULL ok)
    size_t pcm_buffer_size          // Size of PCM buffer in bytes
);
```
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `use_vad` | bool | true | Filter audio through VAD before detection |
...
- `language_token`: Raw token text emitted by the model (e.g. `"<|en|>"`)
...
int cactus_vad(
    ...
    char* response_buffer,          // Buffer for response JSON
    size_t buffer_size,             // Size of response buffer
    const char* options_json,       // Optional VAD options (NULL ok)
    const uint8_t* pcm_buffer,      // Optional raw PCM audio buffer (NULL ok)
    size_t pcm_buffer_size          // Size of PCM buffer in bytes
);
```
...
| `neg_threshold` | float | 0.0 | Non-speech frame threshold; auto-compute as `max(threshold - 0.15, 0.01)` if 0.0 |
| `min_speech_duration_ms` | int | 250 | Minimum speech segment duration (milliseconds) |
| `max_speech_duration_s` | float | infinity | Maximum speech segment duration (seconds) |
| `min_silence_duration_ms` | int | 100 | Minimum silence duration to split segments |
| `speech_pad_ms` | int | 30 | Padding added to each end of segment (milliseconds) |
| `window_size_samples` | int | 512 | Window size for VAD processing |
| `min_silence_at_max_speech` | int | 98 | Minimum silence (milliseconds) at max speech duration split point |
| `use_max_poss_sil_at_max_speech` | bool | true | Use maximum possible silence at max speech duration |
| `sampling_rate` | int | 16000 | Audio sampling rate (Hz) |
...
int cactus_index_add(
    ...
    const char** metadatas,      // Array of metadata JSON strings (NULL ok)
...
int cactus_index_get(
    ...
    size_t* document_buffer_sizes,  // Sizes of document buffers (bytes)
    char** metadata_buffers,     // Output: metadata JSON buffers
    size_t* metadata_buffer_sizes,  // Sizes of metadata buffers (bytes)
    float** embedding_buffers,   // Output: embedding buffers
    size_t* embedding_buffer_sizes  // Sizes of embedding buffers (float elements)
);
```
...
int cactus_index_query(
    ...
    size_t embedding_dim,        // Dimension of each embedding
    const char* options_json,    // Query options JSON
    int** id_buffers,            // Output: arrays of result IDs
    size_t* id_buffer_sizes,     // In: capacity; Out: actual count
    float** score_buffers,       // Output: arrays of similarity scores
    size_t* score_buffer_sizes   // In: capacity; Out: actual count
);
```
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `top_k` | int | 10 | Maximum results per query |
| `score_threshold` | float | -1.0 | Minimum similarity score threshold |
...
- [Cactus Graph API](../cactus_graph/) — Low-level computational graph for custom tensor operations
- [Cactus Index API](../cactus_index/) — On-device vector database for RAG applications
- [Fine-tuning Guide](../finetuning/) — Deploy Unsloth LoRA fine-tunes to mobile
- [Runtime Compatibility](../compatibility/) — Weight versioning across releases
